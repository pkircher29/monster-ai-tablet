  ...
  for (let i = 0; i < plan.length; i++) {
    ...
    try {
      if (!agent || agent.runtime === 'claude-cli') {
        ...
        const codexRes = spawnSync('codex', ['exec', clean(`${sys}\n\n${user}`)], {
          encoding: 'utf8', timeout: 300000, windowsHide: true, shell: process.platform === 'win32', maxBuffer: 8 * 1024 * 1024
        });
        output = (codexRes.stdout || '').trim() || (codexRes.stderr || '').trim() || '(codex executed step)';
      } else {
