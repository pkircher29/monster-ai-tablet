...
const state = {
  ...
  mcpServers: [],
  skillFilter: 'all',
  agora: { rooms: [], currentRoom: 'general-agora' },
  history: [],
  recs: '',
  consensus: { runs: [], jobs: [] },
  caps: null,
...
async function api(path, opts) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const headers = {
      ...(opts?.headers || {}),
      ...(approvalToken ? { 'x-monster-approval': approvalToken } : {}),
    };
    const r = await fetch(routedPath, { ...opts, headers, credentials: 'same-origin' });
    ...
    if (r.status === 428 && detail.approvalId && detail.confirmationPhrase) {
      const confirmation = window.prompt(`Type exactly: ${detail.confirmationPhrase}`);
      if (confirmation !== detail.confirmationPhrase) throw new Error('Action approval cancelled');
      const approval = await fetch('/api/ai-spy/approval', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        credentials: 'same-origin',
        body: JSON.stringify({ approvalId: detail.approvalId, confirmation }),
      });
      if (!approval.ok) throw new Error('Action approval failed');
      approvalToken = (await approval.json()).approvalToken;
      continue;
    }
    throw new Error(detail?.error?.message || path + ' -> ' + r.status);
  }
  throw new Error('Request could not be authorized');
}
...
function derive(snap) {
  ...
  for (const s of subs) {
    ...
    if (!t) {
      if (s.tool !== 'openrouter') continue;
      alerts.push({
        level: 'warn',
        text: `<b>${esc(s.plan)}</b> ${usd(s.monthlyUSD)}/mo — no local usage traces. Check provider dashboard.`,
      });
      continue;
    }
    ...
    if (d === null || d > 30)
      alerts.push({
        level: 'bad',
        text: `<b>${esc(s.plan)}</b> ${usd(s.monthlyUSD)}/mo — ${esc(t.name)} ${d === null ? 'never used' : 'idle ' + d + ' days'}. Cancel candidate.`,
      });
  }
  return {
    cc,
...
function harnessUsageBlock() {
  ...
  if (c) {
    const months = Object.entries(c.byMonth || {})
      .sort()
      .map(([k, v]) => `${k}: ${v}`)
      .join(' · ');
    out += `<div class="panel" style="padding:14px"><b>Codex</b> — ${c.sessions} sessions, ${c.turns} tasks, last ${esc((c.lastActivity || '').slice(0, 10) || 'never')}.
      ...
      <div class="strip-meta">top projects: ${esc(
        Object.keys(c.byProject || {})
          .slice(0, 5)
          .join(', '),
      )}</div></div>`;
  }
...
function liveUsageBlock() {
  ...
  for (const p of u.live) {
    if (p.provider === 'openrouter') {
      if (!p.available) {
        rows.push(['OpenRouter', '—', '—', p.note || 'unavailable', 'warn']);
        continue;
      }
      ...
      const rem =
        p.limitUSD === null
          ? 'pay-as-you-go'
          : p.remainingUSD != null
            ? usd(p.remainingUSD, 2)
            : '—';
      rows.push(['OpenRouter', used + ' spent', rem, p.note || '', 'ok']);
    } else if (p.provider === 'anthropic-api') {
      if (!p.available) {
        rows.push(['Anthropic API', '—', '—', p.note || 'unavailable', 'warn']);
        continue;
      }
      const l = p.limits;
      rows.push([
        'Anthropic API',
        `${l.inputTokensRemaining ?? '?'} / ${l.inputTokensLimit ?? '?'} in-tok`,
        `${l.requestsRemaining ?? '?'} / ${l.requestsLimit ?? '?'} req`,
        'per-minute rate limits (not subscription quota)',
        'ok',
      ]);
    } else if (p.provider === 'ollama') {
      if (!p.available) {
        rows.push(['Ollama', '—', '—', 'not running', 'warn']);
        continue;
      }
      rows.push([
        'Ollama',
        `${p.modelsInstalled} models installed`,
        p.loadedNow?.length ? p.loadedNow.join(', ') + ' loaded' : 'idle',
        'local — no quota',
        'ok',
      ]);
    } else if (p.provider === 'lmstudio') {
      if (!p.available) {
        rows.push(['LM Studio', '—', '—', 'not running', 'warn']);
        continue;
      }
      rows.push([
        'LM Studio',
        `${p.modelsLoaded?.length ?? 0} models loaded`,
        '∞',
        'local — no quota',
        'ok',
      ]);
  ...
  const liveRows = rows
    .map(
      ([prov, used, rem, note, cls]) =>
        `<tr><td>${esc(prov)}</td><td>${esc(used)}</td><td>${esc(rem)}</td><td style="color:var(--dim)">${esc(note)}</td></tr>`,
    )
    .join('');
  const dash = (u.dashboardOnly || [])
    .map(
      (d) =>
        `<tr><td>${esc(d.label)}</td><td colspan="2" style="color:var(--dim)">${esc(d.note)}</td><td><a href="${esc(d.where)}" target="_blank" rel="noopener" style="color:var(--phos)">${esc(d.where.startsWith('http') ? 'open dashboard' : d.where)}</a></td></tr>`,
    )
    .join('');
...
function lineChart(entries, { width = 900, height = 180, color = 'var(--phos)' } = {}) {
  ...
  const ticks = entries
    .filter((_, i) => i % Math.ceil(entries.length / 8) === 0 || i === entries.length - 1)
    .map((e, i, arr) => {
      const gi = entries.indexOf(e);
      return `<text class="axis-t" x="${x(gi).toFixed(1)}" y="${height - 4}" text-anchor="middle">${esc(e.k)}</text>`;
    })
    .join('');
...
const pages = {
  skills(snap) {
    ...
    const filtered =
      filterHarness === 'all'
        ? allSkills
        : allSkills.filter((s) => s.sourceHarness === filterHarness);

    const filterButtons = ['all', 'hermes', 'claude', 'gemini', 'opencode', 'library']
      .map(
        (h) =>
  ...
  agora(snap) {
    ...
    const activeRoom = rooms.find((r) => r.id === agoraState.currentRoom) ||
      rooms[0] || {
        id: 'general-agora',
        title: 'Central Agora',
    ...
    const messages =
      ...
      `;
        ...
        .join('') ||
      '<div class="strip-meta">Room is empty. Send a message or trigger an agent turn!</div>';
  ...
  overview(snap) {
    ...
    const maxMonth = Math.max(
      ...Object.values(d.cc.byMonth || {}).map((v) => v.apiCostUSD || 0),
      1,
    );
    const meters = [
      ledMeter({
        label: 'Value vs plan · this month pace',
        value: d.roi === null ? '—' : `${d.roi.toFixed(1)}<small>×</small>`,
        frac: d.roi === null ? 0 : d.roi / 8,
        invert: true,
        note: `API-equivalent pace ${usd(d.monthPace)}/mo against ${usd(d.claudeSub)}/mo Claude Max — green = earning its cost`,
      }),
      ledMeter({
        label: 'This month · API-equivalent',
        value: `${usd(d.monthEquiv)}`,
        frac: d.monthEquiv / (maxMonth * 1.2),
        note: 'value of tokens burned, priced at API rates',
      }),
      ledMeter({
        label: "Today's burn",
        value: `${usd(d.todayBurn)}`,
        frac: d.todayBurn / Math.max(60, d.todayBurn),
        note: 'API-equivalent today',
      }),
      ledMeter({
        label: 'Declared subscriptions',
        value: `${usd(d.monthlySubs)}<small>/mo</small>`,
        frac: d.monthlySubs / 500,
        note: `${d.subs.filter((s) => s.monthlyUSD > 0).length} paid plans`,
      }),
      ledMeter({
        label: 'Fleet active ≤14d',
        value: `${d.active}<small> / ${d.tools.length}</small>`,
        frac: d.active / Math.max(1, d.tools.length),
        note: `${snap.scan?.dormant?.length ?? 0} dormant installs beyond these`,
      }),
      ledMeter({
        label: 'Idle local disk',
        value: `${d.idleGB.toFixed(0)}<small> GB</small>`,
        frac: d.idleGB / 200,
        note: 'Ollama + LM Studio models on disk',
      }),
    ].join('');
    const last30 = Object.entries(d.cc.byDay || {})
      .sort()
      .slice(-30)
      .map(([k, v]) => ({ k: k.slice(5), v: v.apiCostUSD || 0, title: usd(v.apiCostUSD, 2) }));
    const alerts =
      d.alerts
        .map((a) => `<div class="alert ${a.level === 'warn' ? 'warn' : ''}">${a.text}</div>`)
        .join('') || '<div class="alert warn">No spend alerts. Fleet clean.</div>';
  ...
  usage(snap) {
    const d = derive(snap);
    const models = Object.entries(d.cc.byModel || {}).sort(
      (a, b) => (b[1].apiCostUSD || 0) - (a[1].apiCostUSD || 0),
    );
    ...
    const rows = models
      ...
      <td class="num">${usd(v.apiCostUSD, 2)}<span class="hbar ${m.includes('fable') || m.includes('opus') ? 'amber' : ''}" style="width:${Math.round((70 * (v.apiCostUSD || 0)) / maxC)}px"></span></td></tr>`,
      )
      .join('');
    const months = Object.entries(d.cc.byMonth || {})
      .sort()
      .map(([k, v]) => ({ k, v: v.apiCostUSD || 0, title: usd(v.apiCostUSD) }));
    const projects = Object.entries(d.cc.byProject || {})
      .sort((a, b) => (b[1].apiCostUSD || 0) - (a[1].apiCostUSD || 0))
      .slice(0, 12)
  ...
  fleet(snap) {
    const tools = correctedTools(snap).sort(
      (a, b) => (a.daysSinceUse ?? 9999) - (b.daysSinceUse ?? 9999),
    );
    const strips = tools
      .map((t) => {
        const d = t.daysSinceUse;
      ...
      .join('');
    const dormant = (snap.scan?.dormant || [])
      .map((x) => `<div>${esc(x.dir)}${x.mb ? ` · ${x.mb} MB` : ''}</div>`)
      .join('');
  ...
  spend(snap) {
    ...
    const rows = d.subs
      .map(
        (s) => `<tr><td>${esc(s.tool)}</td><td>${esc(s.plan)}</td>
      <td class="num">${usd(s.monthlyUSD)}</td><td>${verdict(s)}</td></tr>`,
      )
      .join('');
    const cuttable = d.subs
      .filter((s) => s.monthlyUSD && verdict(s).includes('cut'))
      .reduce((a, s) => a + s.monthlyUSD, 0);
  ...
  models(snap) {
    ...
    const body = rows
      .sort((a, b) => (b.bm?.tokPerSec || 0) - (a.bm?.tokPerSec || 0))
      .map(
        (r) => `<tr>
  ...
  consensus() {
    const runs =
      (state.consensus.runs || [])
        .map((r) => {
          ...
          `,
              ...
              const suggestedHtml =
                t.suggestedFollowups && t.suggestedFollowups.length > 0
                  ? `
      ...
      `;
        ...
        .join('') ||
      '<p class="h-sub">No consensus broadcast runs yet. Broadcast a question above to begin.</p>';

    const jobs = (state.consensus.jobs || [])
      .filter((j) => j.status === 'running')
      .map(
        (j) =>
          `<div class="alert warn">Broadcasting to engines: "${esc(j.question.slice(0, 90))}…" (${esc(j.id)})</div>`,
      )
      .join('');
  ...
  ledger() {
    ...
    if (!snap?.claude) return '<h1>Ledger</h1><p class="h-sub">No usage data.</p>';
    const cc = snap.claude,
      b = state.budget || { monthlyTargetUSD: 0, dailyAlertUSD: 0 };
    const now = new Date();
    const thisMonth = now.toISOString().slice(0, 7);
    const dayOfMonth = now.getUTCDate();
    const daysInMonth = new Date(
    ...
    const days = Object.entries(cc.byDay || {})
      .sort()
      .slice(-30)
      .map(([k, v]) => ({ k: k.slice(5), v: v.apiCostUSD || 0 }));
    const months = Object.entries(cc.byMonth || {})
      .sort()
      .map(([k, v]) => ({ k, v: v.apiCostUSD || 0 }));
    // cumulative
    let run = 0;
    const cum = Object.entries(cc.byDay || {})
      .sort()
      .map(([k, v]) => {
        run += v.apiCostUSD || 0;
        return { k: k.slice(5), v: run };
      })
      .slice(-30);
    // model mix
    const models = Object.entries(cc.byModel || {})
      .filter(([m]) => m !== '<synthetic>')
      .sort((a, b) => b[1].apiCostUSD - a[1].apiCostUSD);
    const totalC = models.reduce((a, [, v]) => a + v.apiCostUSD, 0) || 1;

    const todayBurn = cc.byDay?.[now.toISOString().slice(0, 10)]?.apiCostUSD ?? 0;
    const alerts = [];
    if (b.dailyAlertUSD > 0 && todayBurn > b.dailyAlertUSD)
      alerts.push(
        `<div class="alert">Today's API-equivalent burn ${usd(todayBurn, 2)} is over your ${usd(b.dailyAlertUSD)}/day alert.</div>`,
      );
    if (b.monthlyTargetUSD > 0 && projected > b.monthlyTargetUSD)
      alerts.push(
        `<div class="alert">Projected month-end ${usd(projected)} exceeds your ${usd(b.monthlyTargetUSD)}/mo target.</div>`,
      );
    if (b.monthlyTargetUSD > 0 && projected <= b.monthlyTargetUSD)
      alerts.push(
        `<div class="alert info">On track: projected ${usd(projected)} vs ${usd(b.monthlyTargetUSD)}/mo target (${Math.round((projected / b.monthlyTargetUSD) * 100)}%).</div>`,
      );

    const modelBars = models
      .map(
        ([m, v]) => `<tr><td>${esc(m)}</td><td class="num">${usd(v.apiCostUSD, 2)}</td>
      <td class="num">${Math.round((v.apiCostUSD / totalC) * 100)}%<span class="hbar ${m.includes('fable') || m.includes('opus') ? 'amber' : ''}" style="width:${Math.round((60 * v.apiCostUSD) / totalC)}px"></span></td></tr>`,
      )
      .join('');
  ...
  terminal() {
    ...
    const agentOpts = t.targets
      .map(
        (x) =>
          `<option value="${esc(x.id)}"${x.id === t.agentId ? ' selected' : ''}${!x.available && x.kind === 'local' ? '' : ''}>${esc(x.name)}${x.kind === 'local' && !x.available ? ' (offline — will launch)' : ''}</option>`,
      )
      .join('');
    ...
    const bubbles = thread
      .map(
        (m) =>
          `<div class="msg ${m.role}"><div class="msg-who">${m.role === 'user' ? 'you' : esc(m.who || active?.name || 'agent')}${m.seconds != null ? ` · ${m.seconds}s` : ''}</div><div class="msg-body">${esc(m.content)}</div></div>`,
      )
      .join('');
    const pending = t.pending
      ? '<div class="msg assistant"><div class="msg-who">' +
        esc(active?.name || 'agent') +
        '</div><div class="msg-body typing">▮ thinking…</div></div>'
      : '';
  ...
  keyring() {
    ...
    const provOpts =
      Object.entries(k.providers)
        .map(([id, p]) => `<option value="${esc(id)}">${esc(p.label)} → ${esc(p.varName)}</option>`)
        .join('') + '<option value="custom">Custom (name your own var)</option>';
    const targetChecks = Object.entries(k.targets)
      .map(
        ([id, t]) =>
          `<label class="tgt"><input type="checkbox" class="push-tgt" value="${esc(id)}"${id === 'agentos' ? ' checked' : ''}> <b>${esc(t.label)}</b><span>${esc(t.desc)}</span></label>`,
      )
      .join('');
    const rows =
      (k.keys || [])
    ...
    const cards = a.agents
      .map((ag) => {
        ...
        const opts = models
          .map(
            ([id, cfg]) =>
              `<option value="${esc(id)}"${id === ag.activeModel ? ' selected' : ''}>${esc(id)} · ${esc(cfg.role)}${(ag._loaded || []).includes(id) ? ' (loaded)' : ''}</option>`,
          )
          .join('');
        const controls =
          ag.runtime === 'claude-cli'
            ? '<span class="badge ok">orchestrator</span>'
        ...
        const modelPicker =
          ag.runtime === 'claude-cli'
            ? '<div class="strip-meta">CLI default model</div>'
  ...
  command() {
    ...
    if (v.plan && v.catalog) {
      const agentModels = {};
      for (const m of v.catalog) {
        (agentModels[m.agentId] ||= new Set()).add(m.model);
      }
      const agentIds = Object.keys(agentModels);
      body +=
        '<h2>Route — edit before dispatch</h2><div class="panel"><table><tr><th class="num">#</th><th>Step</th><th>Agent</th><th>Model</th></tr>';
      v.plan.forEach((s, i) => {
        const aOpts = agentIds
          .map(
            (a) =>
              `<option value="${esc(a)}"${a === s.agent ? ' selected' : ''}>${esc(a)}</option>`,
          )
          .join('');
        const mOpts = [...(agentModels[s.agent] || agentModels[agentIds[0]] || [])]
          .map(
            (m) =>
      ...
      body +=
        '</table></div><div style="margin:10px 0"><button class="btn" id="vats-exec">Execute route</button> <button class="btn mini" id="vats-cancel">discard</button></div>';
    }
    if (o) {
      if (o.status === 'running')
        body += `<div class="alert warn">Running — ${(o.events || []).filter((e) => e.type === 'step-done').length} steps done…</div>`;
      const doneSteps = (o.events || []).filter((e) => e.type === 'step-done');
      if (doneSteps.length) {
        body += '<h2>Step output</h2>';
        for (const s of doneSteps)
          body += `<div class="run"><b>${esc(s.title)}</b> — ${esc(s.agent)} / ${esc(s.model)} · ${s.seconds}s ${s.error ? '<span class="badge dead">error</span>' : '<span class="badge ok">ok</span>'}<pre>${esc(s.error || s.output || '')}</pre></div>`;
      }
      ...
      if (o.status === 'failed')
        body += `<div class="alert">Failed: ${esc(o.result?.error || 'unknown')}</div>`;
    }
    // past runs
    ...
    if (runs.length) {
      ...
      body += runs
        ...
        .map(
          (
            r,
          ) => `<details class="run"><summary>${esc(r.savedAt?.slice(0, 16).replace('T', ' '))} — ${esc((r.prompt || '').slice(0, 80))}</summary>
        ...
        <button class="btn mini" data-rerun="${esc(r.prompt || '')}">re-run this prompt</button></details>`,
        )
        .join('');
    }
  ...
  network() {
    ...
    const ipLabel = (a) => {
      const parts = [];
      if (a.self) {
        if (a.lanIP) parts.push(a.lanIP);
        if (a.tailscaleIP) parts.push(a.tailscaleIP + ' (TS)');
    ...
    const agentRows =
      ...
      <td>${a.models?.length ? esc(a.models.slice(0, 3).join(', ')) + (a.models.length > 3 ? ` +${a.models.length - 3}` : '') : '—'}</td>
        ...
        .join('') || '<tr><td colspan="6">no agent services discovered</td></tr>';
    ...
    const peerRows =
      ...
      <td>${p.online || !p.lastSeen ? '—' : esc(new Date(p.lastSeen).toISOString().slice(0, 10))}</td>
        ...
        .join('') || '<tr><td colspan="5">tailscale not detected</td></tr>';

    const exposed =
      (n.otherExposed || [])
        .map(
          (l) =>
            `<tr><td class="num">${l.port}</td><td>${esc(l.addr)}</td><td>${esc(l.proc)}</td></tr>`,
        )
  ...
  perks() {
    ...
    const useBadge = (entry) => {
      ...
      const d = entry.lastUsed
        ? Math.floor((Date.now() - new Date(entry.lastUsed)) / 86400000)
        : null;
    ...
    const recRows =
      (c.recommendations || [])
        .map((r, i) => {
          const btn = r.exec
            ? `<button class="btn mini rec-run" data-verb="${esc(r.exec.verb)}" data-target="${esc(r.exec.target)}" data-i="${i}">▶ ${esc(r.exec.label)}</button>`
            : '';
    ...
    const plugRows = (inv.claude?.plugins || [])
      .map((p) => {
    ...
    const skillRows = [...skillNames]
      .map((s) => ({ s, e: u.skills[s] }))
      .sort((a, b) => (b.e?.count ?? 0) - (a.e?.count ?? 0))
      .slice(0, 40)
      .map(
        ({ s, e }) => `<tr><td>${esc(s)}</td>
        <td>${(inv.claude?.skills || []).includes(s) ? 'user-installed' : s.includes(':') ? 'plugin' : 'bundled'}</td>
        <td class="num">${e?.count ?? 0}</td><td>${lastOf(e)}</td><td>${useBadge(e)}</td></tr>`,
      )
      .join('');

    // mcp servers observed in transcripts + configured
    const mcpNames = new Set([
      ...(inv.claude?.mcpServers || []).map((m) => m.name),
      ...Object.keys(u.mcp),
    ]);
    const mcpRows = [...mcpNames]
      .map((n) => ({ n, e: u.mcp[n] }))
      .sort((a, b) => (b.e?.count ?? 0) - (a.e?.count ?? 0))
      .slice(0, 40)
      .map(({ n, e }) => {
        const pretty =
          n.length > 30 && /^[0-9a-f-]{30,}$/.test(n)
            ? n.slice(0, 8) + '… (claude.ai connector)'
            : n;
    ...
    // subagents
    const agentRows = Object.entries(u.agents)
      .sort((a, b) => b[1].count - a[1].count)
      .map(
        ([n, e]) =>
          `<tr><td>${esc(n)}</td><td class="num">${e.count}</td><td>${lastOf(e)}</td><td>${useBadge(e)}</td></tr>`,
      )
      .join('');

    // codex side
    const cl = new Set((inv.claude?.plugins || []).map((p) => p.name));
    const codexRows = (inv.codex?.plugins || [])
      .map(
        (p) =>
          `<tr><td>${esc(p.name)}</td><td>${esc(p.marketplace)}</td>
       <td>${p.enabled ? '<span class="badge ok">enabled</span>' : '<span class="badge">off</span>'}</td>
       <td>${cl.has(p.name) ? '<span class="badge ok">also in claude</span>' : '<span class="badge">codex only</span>'}</td></tr>`,
      )
      .join('');
...
function wireSkills() {
  document.querySelectorAll('[data-skill-filter]').forEach((b) =>
  ...
  // Deploy button
  document.querySelectorAll('[data-skill-deploy]').forEach((b) =>
    b.addEventListener('click', async () => {
      ...
      try {
        ...
        if (res.ok) {
          alert(
            `Skill "${key}" successfully transmuted & deployed to ${toHarness.toUpperCase()}! Target path: ${res.destPath}`,
          );
  ...
  // Inspect / Edit modal
  document.querySelectorAll('[data-skill-inspect]').forEach((b) =>
    b.addEventListener('click', () => {
    ...
    `;
      ...
      document.getElementById('modal-save-skill')?.addEventListener('click', async () => {
        ...
        const res = await post('/api/skills/save', {
          key: skill.key,
          name,
          description,
          content,
...
function wireAgora() {
  ...
  // Select room
  document.querySelectorAll('.agora-room-item').forEach((el) =>
  ...
  document.getElementById('new-room-btn')?.addEventListener('click', () => {
    ...
    document.getElementById('modal-create-room-submit')?.addEventListener('click', async () => {
      ...
      const participants = [...document.querySelectorAll('.agora-part-chk:checked')].map(
        (c) => c.value,
      );
  ...
  const sendHuman = async () => {
    ...
    if (!text) return;
    inp.value = '';
    await post('/api/agora/message', {
      roomId: agoraState.currentRoom,
      text,
      sender: 'You (Human)',
      role: 'human',
    });
...
function render() {
  ...
  if (!state.snap) {
    main.innerHTML = '<div class="loading">Reading the board…</div>';
    return;
  }
  // preserve in-progress form input and open <details> across re-renders
  const drafts = {};
  let focusId = document.activeElement?.id || null;
  for (const el of main.querySelectorAll('input[id], textarea[id]')) drafts[el.id] = el.value;
  const open = new Set(
    [...main.querySelectorAll('details[open] > summary')].map((s) => s.textContent),
  );
  ...
  if (lanLink)
    lanLink.addEventListener('click', async (e) => {
      e.preventDefault();
      lanLink.textContent = 'sweeping 254 hosts…';
      try {
        state.net = await api('/api/network?lan=1');
        render();
      } catch {
        lanLink.textContent = 'sweep failed';
      }
    });
...
function wireConsensus() {
  ...
  document.querySelectorAll('[data-suggest-q]').forEach((chip) =>
  ...
  document.querySelectorAll('[data-followup-run]').forEach((btn) =>
  ...
  document.querySelectorAll('[data-run-input]').forEach((inp) =>
...
function wireGarage() {
  const setStatus = (id, msg, cls = '') => {
    ...
    if (el) el.innerHTML = `<span class="badge ${cls}">${esc(msg)}</span>`;
  };
  document.querySelectorAll('.ag-name').forEach((inp) =>
    inp.addEventListener('blur', async () => {
      const id = inp.dataset.nameFor;
      await post('/api/agents/rename', { id, name: inp.value }).catch(() => {});
    }),
  );
  document.querySelectorAll('.ag-desc').forEach((t) =>
    t.addEventListener('blur', async () => {
      const id = t.dataset.descFor;
      await post('/api/agents/describe', { id, description: t.value }).catch(() => {});
    }),
  );
  document.querySelectorAll('[data-act]').forEach((btn) =>
    btn.addEventListener('click', async () => {
      ...
      try {
        if (act === 'launch') {
          setStatus(id, 'launching…', 'stale');
          const r = await post('/api/agents/launch', { id });
          setStatus(
            id,
            r.ok
              ? r.alreadyRunning
          ...
          setStatus(id, 'restarting…', 'stale');
          const r = await post('/api/agents/restart', { id });
          setStatus(id, r.ok ? 'restarted' : 'failed: ' + (r.error || ''), r.ok ? 'ok' : 'dead');
        } else if (act === 'setmodel') {
          const sel = document.querySelector(`[data-model-for="${id}"]`);
          const model = sel.value;
          setStatus(id, 'loading ' + model + '…', 'stale');
          const r = await post('/api/agents/model', { id, model });
          setStatus(
            id,
            r.ok ? 'active: ' + r.activeModel : 'failed: ' + (r.error || ''),
            r.ok ? 'ok' : 'dead',
          );
        }
...
function wireLedger() {
  $('#bud-save')?.addEventListener('click', async () => {
    ...
    state.budget = await post('/api/budget', { monthlyTargetUSD, dailyAlertUSD }).catch(
      () => state.budget,
    );
    render();
  });
...
function wireWorkshop() {
  document.querySelectorAll('[data-bench]').forEach((b) =>
    b.addEventListener('click', async () => {
      ...
      b.disabled = true;
      b.textContent = 'benching…';
      const r = await post('/api/benchmark', { agentId, model }).catch((e) => ({
        ok: false,
        error: e.message,
      }));
      if (r.ok)
        state.benchmarks = (
          await api('/api/benchmark').catch(() => ({ results: state.benchmarks }))
        ).results;
      else {
  ...
  $('#bench-all')?.addEventListener('click', async () => {
    const btn = $('#bench-all');
    btn.disabled = true;
    btn.textContent = 'benchmarking all (slow)…';
    try {
      ...
      const t = setInterval(async () => {
        ...
        if (job.status !== 'running') {
          clearInterval(t);
          state.benchmarks = (await api('/api/benchmark')).results;
          render();
        }
      }, 3000);
    } catch {
...
function wireTerminal() {
  ...
  const send = async () => {
    ...
    render();
    try {
      const { jobId } = await post('/api/chat', {
        agentId: t.agentId,
        model,
        messages: t.threads[key].map((m) => ({ role: m.role, content: m.content })),
      });
...
async function pollChat(id, key, who) {
  ...
  const timer = setInterval(async () => {
    try {
      ...
      t.threads[key].push({
        role: 'assistant',
        content: r.ok ? r.reply : 'error: ' + (r.error || 'failed'),
        who,
        seconds: r.seconds,
      });
      t.pending = false;
...
function wireKeyring() {
  ...
  if (prov)
    prov.addEventListener('change', () => {
      varInp.style.display = prov.value === 'custom' ? '' : 'none';
    });
  const selectedTargets = () =>
    [...document.querySelectorAll('.push-tgt:checked')].map((c) => c.value);
  $('#k-add')?.addEventListener('click', async () => {
    const provider = prov.value,
      value = $('#k-value').value.trim(),
    ...
    state.keys = await api('/api/keys').catch(() => state.keys);
    render();
  });
  document.querySelectorAll('[data-key-push]').forEach((b) =>
    b.addEventListener('click', async () => {
      const id = b.dataset.keyPush;
      const targets = selectedTargets();
      if (!targets.length) return alert('tick at least one push target in the Add-a-key box first');
      b.disabled = true;
      b.textContent = 'pushing…';
      const r = await post('/api/keys/push', { id, targets }).catch((e) => ({
        ok: false,
        error: e.message,
      }));
      const msg = r.results
        ? Object.entries(r.results)
            .map(
              ([t, x]) =>
                `${t}: ${x.ok ? 'ok' + (x.note ? ' (' + x.note + ')' : '') : 'FAIL ' + x.error}`,
            )
            .join('\n')
        : r.error || 'failed';
      alert(msg);
  ...
  document.querySelectorAll('[data-key-del]').forEach((b) =>
    b.addEventListener('click', async () => {
      if (
        !confirm(
          'Delete this key from the store? (does not remove it from where it was already pushed)',
        )
      )
        return;
...
function wirePerks() {
  document.querySelectorAll('.rec-run').forEach((btn) =>
    btn.addEventListener('click', async () => {
      ...
      if (
        !confirm(
          `Run: claude plugin ${verb} ${target}?\n\n${verb === 'disable' ? 'Reversible — you can re-enable it anytime.' : 'Enables the plugin if it is installed.'}`,
        )
      )
        return;
      btn.disabled = true;
      ...
      try {
        ...
        if (out)
          out.innerHTML = r.ok
            ? ` <span class="badge ok">done: ${esc(r.output || verb + 'd')}</span>`
            : ` <span class="badge dead">${esc(r.error || 'failed')}</span>`;
        if (r.ok) {
          btn.textContent = verb === 'disable' ? '✓ disabled' : '✓ enabled';
          state.caps = await api('/api/capabilities?refresh=1').catch(() => state.caps);
        } else btn.disabled = false;
      } catch (e) {
        if (out) out.innerHTML = ` <span class="badge dead">${esc(e.message)}</span>`;
        btn.disabled = false;
      }
    }),
  );
...
function wireCommand() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      state.orch = { status: 'running', prompt, events: [] };
      state.vats.plan = null;
      inp.value = '';
      render();
      pollOrch(jobId);
    } catch (e) {
...
function wireVats() {
  ...
  $('#vats-plan')?.addEventListener('click', async () => {
    const prompt = $('#orch-q').value.trim();
    if (!prompt) return;
    const b = $('#vats-plan');
    b.disabled = true;
    b.textContent = 'planning…';
    const r = await post('/api/orchestrate/plan', { prompt }).catch((e) => ({
      ok: false,
      error: e.message,
    }));
    if (r.ok) {
  ...
  document.querySelectorAll('[data-step-agent]').forEach((sel) =>
    sel.addEventListener('change', () => {
      const i = +sel.dataset.stepAgent;
      v.plan[i].agent = sel.value;
  ...
  document.querySelectorAll('[data-step-model]').forEach((sel) =>
    sel.addEventListener('change', () => {
  ...
  $('#vats-exec')?.addEventListener('click', async () => {
    ...
    state.orch = { status: 'running', prompt: v.prompt, events: [] };
    v.plan = null;
    render();
    pollOrch(jobId);
  });
  $('#vats-cancel')?.addEventListener('click', () => {
    v.plan = null;
    render();
  });
  document.querySelectorAll('[data-rerun]').forEach((b) =>
    b.addEventListener('click', async () => {
      const prompt = b.dataset.rerun;
      const { jobId } = await post('/api/orchestrate', { prompt });
      state.orch = { status: 'running', prompt, events: [] };
      render();
      pollOrch(jobId);
  ...
  const t = setInterval(async () => {
    try {
      ...
      if (job.status !== 'running') {
        clearInterval(t);
        state.orchRuns = (
          await api('/api/orchestrate/runs').catch(() => ({ runs: state.orchRuns }))
        ).runs;
      }
...
async function loadAll({ refresh = false } = {}) {
  try {
    ...
    state.recs = recs.markdown;
    state.consensus = consensus;
    state.caps = caps;
    state.net = net;
    state.usageLive = usageLive;
...
function applyLite(on) {
  ...
  const b = $('#lite-btn');
  if (b) b.textContent = on ? 'CRT mode' : 'Lite mode';
  localStorage.setItem('agentos-lite', on ? '1' : '0');
}
$('#lite-btn')?.addEventListener('click', () =>
  applyLite(!document.body.classList.contains('lite')),
);
