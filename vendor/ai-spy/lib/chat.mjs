...
export async function chatTargets() {
  ...
  targets.push({
    ...
    description: 'Anthropic Claude via CLI. Frontier reasoning, planning, code architecture.',
  });
  ...
  targets.push({
    ...
    kind: 'agent',
    available: true,
    models: ['hermes-3', 'nous-hermes-2', 'open-hermes'],
    description:
      'Nous Hermes Agent. Autonomous tool execution, subagent orchestration, state persistence.',
  });
  ...
  if (onPath('codex')) {
    targets.push({
      ...
      kind: 'cli',
      available: true,
      models: ['gpt-5-codex', 'codex-pro'],
      description: 'OpenAI Codex CLI. Code-focused agent.',
    });
  }
...
function flatten(messages, latest) {
  ...
  return (
    (prior
      ? `Ongoing conversation — reply only as the assistant's next message, no preamble.\n\n${prior}\n\n`
      : '') + `User: ${latest}`
  );
}
...
function cliChat(bin, args, timeout = 2500) {
  const r = spawnCliSync(bin, args, {
    encoding: 'utf8',
    timeout,
    maxBuffer: 8 * 1024 * 1024,
  });
  if (r.status !== 0 && !r.stdout)
    throw new Error((r.stderr || r.error?.message || bin + ' failed').slice(0, 300));
  ...
  try {
    ...
    if (normalizedId === 'claude' && onPath('claude')) {
      ...
      if (existsSync(hermesScript)) {
        try {
          const r = spawnSync('python3', [hermesScript, 'chat', '--once', clean(latest)], {
            encoding: 'utf8',
            timeout: 2500,
          });
...
function generateAgentDebateTurn(agentId, prompt, history) {
  ...
  const historyText = Array.isArray(history)
    ? history.map((m) => m.content || m.text || '').join(' ')
    : String(prompt);
