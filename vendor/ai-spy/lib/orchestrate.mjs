...
function claude(prompt, { model = 'sonnet', timeout = 300000 } = {}) {
  const r = spawnCliSync('claude', ['-p', clean(prompt), '--model', model], {
    encoding: 'utf8',
    timeout,
    maxBuffer: 8 * 1024 * 1024,
  });
  if (r.status !== 0 && !r.stdout)
    throw new Error((r.stderr || r.error?.message || 'claude failed').slice(0, 300));
...
export async function planTask(rawPrompt) {
  ...
  const planPrompt = `You are the orchestrator for a fleet of local AI agents. Break the user's task into 1-5 sequential steps and assign each step to the single best agent+model from the menu. Prefer local agents (ollama, lmstudio) for the work; reserve the claude agent for planning/synthesis or steps that truly need frontier reasoning. Match role to task: coder for code, reasoning for logic/math, vision for images, general otherwise. Avoid offline servers if an online option fits.
...
Reply with ONLY a JSON array, no prose. Each element: {"title": short step name, "agent": agentId, "model": model string, "instruction": what this agent should do for this step}. Keep instructions self-contained.`;
  ...
  let planText;
  try {
    planText = claude(planPrompt);
  } catch (e) {
    return { ok: false, error: 'planner failed: ' + e.message };
  }
  let plan = parsePlan(planText);
  if (!plan || !Array.isArray(plan) || !plan.length) {
    const fb =
      catalog.find((m) => m.running && m.role === 'general') ||
      catalog.find((m) => m.running) ||
      catalog[0];
    plan = [
      {
        title: 'Answer',
        agent: fb?.agentId || 'claude',
        model: fb?.model || 'sonnet',
        instruction: prompt,
      },
    ];
  }
...
export async function executePlan(rawPrompt, plan, { onEvent = () => {} } = {}) {
  ...
  for (let i = 0; i < plan.length; i++) {
    const step = plan[i];
    const agent =
      reg.agents.find((a) => a.id === step.agent) ||
      reg.agents.find((a) => a.runtime === 'claude-cli');
    onEvent({
      type: 'step-start',
      index: i,
      title: step.title,
    ...
    try {
      if (!agent || agent.runtime === 'claude-cli') {
        output = claude(`${sys}\n\n${user}`, {
          model: /opus|sonnet|haiku|fable/.test(step.model) ? step.model : 'sonnet',
        });
      } else if (agent.runtime === 'codex-cli') {
        const codexRes = spawnCliSync('codex', ['exec', clean(`${sys}\n\n${user}`)], {
          encoding: 'utf8',
          timeout: 300000,
          maxBuffer: 8 * 1024 * 1024,
        });
        output =
          (codexRes.stdout || '').trim() ||
          (codexRes.stderr || '').trim() ||
          '(codex executed step)';
      } else {
        if (agent.port) {
          const up = await launch(agent.id);
          if (!up.ok) throw new Error(`could not start ${agent.name}`);
        }
  ...
  if (good.length > 1) {
    ...
    try {
      final = claude(
        `You orchestrated these steps for the user task "${prompt}". Synthesize one clear, complete final answer. Do not mention the internal steps unless relevant.\n\nSTEPS:\n${context}`,
      );
    } catch {
      final = good[good.length - 1].output;
    }
