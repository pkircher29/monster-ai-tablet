...
lines. The endpoint is read-only and does not launch an agent session. Each successful snapshot is
stored in a capped host-local SQLite history containing only the closed response fields.
